...
* Fehlerkorrektur - Antwort-Token können jetzt gesendet werden in `at-content-rendering-failed` -Ereignisse.
...
* Unterstützung für die Einstellung benutzerdefinierter [!DNL Adobe Experience Platform] (AEP) `sandboxId` und `sandboxName` in `targetGlobalSettings`, der an die Bereitstellungs-API am `getOffer/getOffers` -Aufrufe.
* Shadow DOM-Korrektur für Verkettung `:eq()` in Selektoren.
...
* Fest `pageLoad` nicht im Hybridausführungsmodus On Device Decisioning (ODD) mit target-global-mbox verknüpft werden.
...
* Die Cache-Initialisierung der Einzelseiten-App (SPA)-Ansicht wird jetzt korrekt verarbeitet und berücksichtigt `viewsEnabled` -Einstellungen. Einstellung `viewsEnabled` der `false` Wert deaktiviert jetzt die `triggerView()` -Funktion. Siehe [Reihenfolge der Vorgänge beim ersten Laden der Seite](/help/dev/implement/client-side/atjs/how-to-deployatjs/target-atjs-single-page-application.md#order).
...
* Benachrichtigungen, die angezeigt wurden, wurden von Click-Tracking-Ereignishandlern entkoppelt. Zunächst [!DNL Target] keine Benachrichtigungen gesendet, wenn Click-Event-Handler, die zu einer gerenderten Ansicht gehören, nicht angehängt werden konnten. [!DNL Target] sendet jetzt eine Ansichtsbenachrichtigung, selbst wenn Klickelemente nicht gefunden werden. (TNT-31969)
...
* [!DNL Target] erzwingt jetzt, dass die `mbox` Schlüssel in der `adobe.target.applyOffer(options)` verwendet wird. Dieser Schlüssel war in der Vergangenheit erforderlich, aber [!DNL Target] erzwingt nun seine Verwendung, um sicherzustellen, dass [!DNL Target] hat eine ordnungsgemäße Validierung und Kunden verwenden die Funktion ordnungsgemäß.
...
* Es wurde ein Problem beim Klick-Tracking auf einen Link mit `target="_blank"` verhindert [!DNL Target] durch Öffnen des Links in einer neuen Registerkarte. (TNT-28072)
...
* Unterstützung für Umleitungsangebote für A4T. Nach dem Herunterladen und Installieren von at.js Version 0.9.6 können Sie Umleitungsangebote in Aktivitäten verwenden, die [!UICONTROL Adobe Analytics als Berichtsquelle für Target] (A4T). Neben Version 0.9.6 von at.js gibt es weitere Mindestanforderungen für Ihre Implementierung, um Umleitungsangebote und A4T nutzen zu können. Weitere wichtige Informationen finden Sie unter [Umleitungsangebote – A4T-FAQ](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/a4t-faq/a4t-faq-redirect-offers.html).
...
* Es wurde die Einstellung `optoutEnabled` hinzugefügt, um die Abmeldung für Gerätediagramme zu ermöglichen. Ist diese Option als `true` eingestellt und hat sich der Besucher gegen eine Verfolgung entschieden, ruft der Browser des Benutzers die Mbox nicht auf. Gerätediagramme sind derzeit als Betaversion verfügbar. Diese Einstellung ist auf `false` Standardmäßig, muss jedoch auf `true` , wenn Sie Gerätediagramme verwenden.
...
at.js ist eine neue Implementierungsbibliothek für [!DNL Target] entwickelt für typische Webimplementierungen und Einzelseitenanwendungen.
...
* Die asynchrone Implementierung bedeutet veraltete Integrationen wie die [!UICONTROL Test&amp;Target auf SiteCatalyst] -Plug-in funktioniert möglicherweise nicht.
